import { decryptApiKey, delKV, encryptApiKey, executeQuery, returnError, returnSuccess, tryCatchErrorToString, } from "../../../core/index.js";
...
export const api = {
    ...
    handler: async (reqCtx, ctx) => {
        try {
            ...
            if (method === "GET") {
                const userRes = await executeQuery("SELECT ai_base_url, ai_model_name, ai_api_key FROM users WHERE id = ? LIMIT 1", [currentUserId]);
                if (userRes.status === 0) {
            ...
            if (method === "POST" || method === "PUT") {
                ...
                // 先查出当前存储的旧 Key
                const oldUserRes = await executeQuery("SELECT ai_api_key FROM users WHERE id = ? LIMIT 1", [currentUserId]);
                ...
                const updateRes = await executeQuery("UPDATE users SET ai_base_url = ?, ai_model_name = ?, ai_api_key = ? WHERE id = ?", [cleanBaseUrl, rawModelName, finalEncryptedKey, currentUserId]);
