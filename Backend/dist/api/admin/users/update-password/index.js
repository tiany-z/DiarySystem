import { delKV, executeQuery, hashPassword, returnError, returnSuccess, SUPER_ADMIN_USERNAME, } from "#core";
export const api = {
    ...
    handler: async (reqCtx, ctx) => {
        try {
            ...
            // 1. 检查目标用户是否存在
            const userRes = await executeQuery("SELECT id, username FROM users WHERE id = ? LIMIT 1", [cleanUserId]);
            if (userRes.status === 0)
