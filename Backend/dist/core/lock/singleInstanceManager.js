...
class SingleInstanceManagerImpl {
    localLockPath = path.resolve(process.cwd(), ".instance.lock");
    isLocalLocked = false;
    clusterKey = "backend:single_instance:lock";
    renewalTimer = null;
    currentNodeId = "diary-backend-01";
    currentPort = 8000;
    ...
    acquireLocalLock(options) {
        try {
            ...
            if (fs.existsSync(this.localLockPath)) {
                ...
                if (existingMeta && existingMeta.pid) {
                    ...
                    if (this.isProcessAlive(existingMeta.pid)) {
                        return returnError(`本地已存在运行中的后端实例 (PID: ${existingMeta.pid}, Node: ${existingMeta.nodeId}, 启动时间: ${existingMeta.startedAt})`);
                    }
                    else {
                        LogClient.warn(`检测到非正常退出遗留的陈旧本地实例锁 (原 PID: ${existingMeta.pid})，正在自愈清理并接管...`, undefined, "SingleInstance");
                        try {
            ...
            LogClient.info(`成功获取本地单实例独占锁 (PID: ${meta.pid}, 文件: ${this.localLockPath})`, undefined, "SingleInstance");
     ...
     */
    async acquireClusterLock(_nodeId, _port = 8000) {
        return returnSuccess(true);
    }
    /**
     * 释放集群分布式锁（单进程架构下空实现）
     */
    async releaseClusterLock() {
        return returnSuccess(true);
    }
    /**
     * 统一释放所有单实例互斥锁
...
//# sourceMappingURL=singleInstanceManager.js.map