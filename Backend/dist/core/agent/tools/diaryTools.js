...
// =========================================================================
export const searchDiariesTool = {
    type: "function",
    function: {
        name: "search_diaries",
...
export const searchDiariesExecutor = async (args, context) => {
    try {
...
// =========================================================================
export const locateDiaryContentTool = {
    type: "function",
    function: {
...
export const locateDiaryContentExecutor = async (args, context) => {
    try {
    ...
    `;
        ...
        for (const row of rows) {
            if (locations.length >= safeMaxMatches)
                break;
            const content = row.content || "";
            if (!content.includes(targetKeyword))
                continue;
            let currentOffset = 0;
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                if (lineKeywordIdx !== -1) {
                    ...
                    const highlightedSnippet = snippetText.replace(targetKeyword, `【${targetKeyword}】`);
                    locations.push({
                        diaryId: row.id,
...
// =========================================================================
export const readDiaryDetailTool = {
    type: "function",
    function: {
...
export const readDiaryDetailExecutor = async (args, context) => {
    try {
        const { diaryId } = args || {};
        if (!diaryId || typeof diaryId !== "string" || !diaryId.trim()) {
            return {
        ...
        // 1. 尝试从内存缓存命中
        const cachedRes = await getKV("diaries", cleanId);
      ...
      LIMIT 1;
    `;
        const dbRes = await executeQuery(sql, [cleanId, context.userId]);
        if (dbRes.status === 0) {
            return {
                success: false,
...
// =========================================================================
export const getDiaryTimelineStatsTool = {
    type: "function",
    function: {
...
export const getDiaryTimelineStatsExecutor = async (args, context) => {
    try {
    ...
    `;
        const summaryRes = await executeQuery(summarySql, [context.userId]);
      ...
      ORDER BY count DESC;
    `;
        const moodRes = await executeQuery(moodSql, [context.userId]);
        const moodDistribution = (moodRes.data || []).map((m) => ({
            mood: m.mood || "未知",
      ...
      ORDER BY count DESC;
    `;
        const weatherRes = await executeQuery(weatherSql, [context.userId]);
        const weatherDistribution = (weatherRes.data || []).map((w) => ({
      ...
      GROUP BY writeHour;
    `;
        const hourRes = await executeQuery(hourSql, [context.userId]);
        let morning = 0; // 5-9点
        let daytime = 0; // 9-18点
        ...
        const peakWritingPeriod = totalNotes > 0 && topPeriod && topPeriod.count > 0
...
export const createDiaryExecutor = async (args, context) => {
    try {
...
export const updateDiaryExecutor = async (args, context) => {
    ...
    const cleanId = diaryId.trim();
    // 1. 申请行级排他锁保护
    const lockAcq = await RowLockManager.acquireRowLock("diaries", cleanId, "UPDATE", context.requestId, 10000);
    if (lockAcq.status === 0) {
        return {
            success: false,
    ...
    try {
        // 2. 抓取旧快照并核验所有权
        const selectRes = await executeQuery("SELECT * FROM diaries WHERE id = ? AND user_id = ? AND deleted_at IS NULL LIMIT 1;", [cleanId, context.userId]);
    ...
    `;
        ...
        const undoFn = async () => {
            await executeQuery("UPDATE diaries SET title = ?, content = ?, weather = ?, mood = ?, updated_at = ? WHERE id = ?", [oldRow.title, oldRow.content, oldRow.weather, oldRow.mood, oldRow.updated_at, cleanId]).catch(() => { });
        ...
        const updatedFields = [];
        if (newTitle !== oldRow.title)
            updatedFields.push("title");
        if (newContent !== oldRow.content)
            updatedFields.push("content");
        if (newWeather !== oldRow.weather)
            updatedFields.push("weather");
        if (newMood !== oldRow.mood)
            updatedFields.push("mood");
...
export const deleteDiaryExecutor = async (args, context) => {
    try {
        ...
        if (confirmed !== true) {
            return {
                ...
                summary: "需要用户二次确认",
                data: {
                    message: "⚠️ 警告：删除日记属于不可逆的破坏性操作。请向用户明确确认：‘确认要永久删除这篇日记吗？’，待用户确认后再传入 confirmed: true 执行删除。",
                },
            };
        }
        // 1. 申请行级排他锁保护
        const lockAcq = await RowLockManager.acquireRowLock("diaries", cleanId, "DELETE", context.requestId, 10000);
        if (lockAcq.status === 0) {
            return {
                success: false,
                error: `获取日记行锁失败: ${lockAcq.content}`,
        ...
        try {
            // 2. 核验归属权
            const selectRes = await executeQuery("SELECT id, title FROM diaries WHERE id = ? AND user_id = ? AND deleted_at IS NULL LIMIT 1;", [cleanId, context.userId]);
            ...
            // 5. 绑定撤销闭包
            const undoFn = async () => {
                await executeQuery("UPDATE diaries SET deleted_at = NULL WHERE id = ? AND user_id = ?", [cleanId, context.userId]).catch(() => { });
...
//# sourceMappingURL=diaryTools.js.map