export const webSearchTool = {
    ...
    function: {
        name: "web_search",
        description: "进行实时互联网网页搜索。用于检索最新外部资讯、时事、技术资料、天气与百科知识。当用户提问涉及外部世界或最新信息时调用此工具。",
        parameters: {
            type: "object",
            properties: {
                query: {
                    type: "string",
                    description: "精炼的搜索查询词。避免自然语言长句，提取核心关键词短语（如 '杭州 明天 天气' 或 'Node.js LTS 最新版本'）",
                },
                maxResults: {
                    type: "integer",
...
function parseDuckDuckGoHtml(html, limit) {
    ...
    while ((match = resultRegex.exec(html)) !== null && results.length < limit) {
        ...
        if (rawUrl.includes("uddg=")) {
            try {
                const u = new URL(rawUrl.startsWith("http") ? rawUrl : `https://duckduckgo.com${rawUrl}`).searchParams.get("uddg");
                if (u) {
...
export const webSearchExecutor = async (args, _context) => {
    ...
    return {
        ...
        data: {
            ...
            total: 0,
            results: [],
            notice: "当前外部网络环境受限或搜索引擎未返回有效结果。请结合已有知识作答，并向用户说明无法获取实时最新网页。",
        },
