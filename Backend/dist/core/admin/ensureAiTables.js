 ...
 */
export async function ensureUserAiConfigColumns() {
    try {
        ...
        const existingColumns = new Set(checkRes.data.map((col) => col.Field));
        ...
        if (missingStatements.length > 0) {
            ...
            LogClient.success(`🤖 users 表已自愈补充 AI 模型配置字段: [${missingStatements.join(", ")}]`, undefined, "AdminInit");
        }
        else {
