...
import { declare, insert, parameterizeSql, parseColumns, parseWhere, remove, RowLockManager, MemoryCache, select, update, validateSQLFragment, WsResilienceQueue, GlobalStore, buildHeartbeatPayload, parseHeartbeatPayload, parseWsMessage, stringifyWsMessage, countTokensFallback, truncateByTokensFallback, } from "../core/index.js";
function ok(result) {
    if (result.status !== 1) {
...
describe("DiarySystem/Backend - MySQL SQL AST & Builders", () => {
    ...
    describe("WsResilienceQueue (WS 软断连缓冲队列)", () => {
        it("应支持软断连宽限期内的消息暂存与重连补发", async () => {
            ...
            const sendPromise = queue.enqueuePendingMessage("msg-1", { text: "hello diary" });
            const sentMessages = [];
...
//# sourceMappingURL=sqlBuilderMysql.test.js.map