...
describe("Agent Tools & Registry Test Suite", () => {
    ...
    it("should reject delete_diary if confirmed is not true", async () => {
        const res = await ToolRegistry.executeTool("delete_diary", { diaryId: "d-123", confirmed: false }, { userId: "user-123", requestId: "req-1" });
        expect(res.success).toBe(false);
    ...
    it("should allow dynamic tool registration", async () => {
        ...
        const result = await ToolRegistry.executeTool("mock_test_tool", JSON.stringify({ msg: "hello" }), { userId: "user-1", requestId: "req-1" });
