...
export function sanitizeIp(rawIp) {
...
export function parseForwardedHeader(headerValue) {
    if (!headerValue)
        return [];
...
export function parseRfcForwardedHeader(headerValue) {
    if (!headerValue)
        return [];
    const rawList = Array.isArray(headerValue) ? headerValue : [headerValue];
    const ips = [];
...
export function isPrivateIp(ip) {
    ...
    if (clean.startsWith("192.168."))
        return true;
    if (/^172\.(1[6-9]|2\d|3[01])\./.test(clean))
        return true;
    if (clean.startsWith("169.254."))
        return true;
    if (/^100\.(6[4-9]|[7-9]\d|1[01]\d|12[0-7])\./.test(clean))
        return true;
