...
async function main() {
    ...
    if (localLockRes.status === 0) {
        printStartupError("DiaryBackend [单实例冲突拦截]", `无法启动当前实例，系统已被另一个运行中的实例锁定:\n${localLockRes.content}\n提示: 该系统被配置为严格单实例模式，请停止现有实例后再启动，或检查并清理陈旧锁。`);
        process.exit(1);
    }
    ...
    if (dbPingRes.status === 0) {
        LogClient.warn(`[MySQL 连通性提示] 无法立即连接至 MySQL [${process.env.MYSQL_HOST}:${process.env.MYSQL_PORT}]: ${dbPingRes.content}。请在运行业务查询前确保 MySQL 服务可用。`, undefined, "DiaryBackend");
    }
    else {
    ...
    await new Promise((resolve, reject) => {
        ...
        httpServer.on("error", async (err) => {
            if (err.code === "EADDRINUSE") {
                await SingleInstanceManager.releaseAll();
                printStartupError("DiaryBackend [网络端口冲突拦截]", `HTTP 监听端口 ${httpPort} 已被占用 (EADDRINUSE)。已自动清理本进程单实例锁。`);
                process.exit(1);
            }
            reject(err);
        });
