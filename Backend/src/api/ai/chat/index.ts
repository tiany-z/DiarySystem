...
export const api = {
  ...
  authRequired: true,
  handler: async (
    reqCtx: { req: any; res?: any; body?: any; user?: any },
    ctx: any
    ...
    const userRes = await executeQuery<any>(
      "SELECT username, nickname, ai_base_url, ai_model_name, ai_api_key FROM users WHERE id = ? LIMIT 1;",
      [currentUserId]
    );
    ...
    if (!rawApiKey || !baseUrl || !modelName) {
      return returnError(
        "请先在右上角「个人资料 -> AI 助手设置」中配置大模型的 Base URL、Model Name 与 API Key"
      );
    }
    ...
    let convTitle = "新对话";
    if (conversationId) {
      const convCheck = await executeQuery<any>(
        "SELECT id, title FROM ai_conversations WHERE id = ? AND user_id = ? AND deleted_at IS NULL LIMIT 1;",
      ...
      const createConvRes = await executeQuery(
        "INSERT INTO ai_conversations (id, user_id, title) VALUES (?, ?, ?);",
        [conversationId, currentUserId, convTitle]
      );
    ...
    const saveUserMsgRes = await executeQuery(
      "INSERT INTO ai_messages (id, conversation_id, user_id, role, content) VALUES (?, ?, ?, 'user', ?);",
      [userMsgId, conversationId, currentUserId, message]
    );
    ...
    const historyRes = await executeQuery<any>(
    ...
    const chatMessages: AgentChatMessage[] = historyRows
