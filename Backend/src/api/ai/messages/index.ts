...
export const api = {
  ...
  authRequired: true,
  handler: async (
    reqCtx: { req: any; query?: any; user?: any },
    ctx: any
    ...
    try {
      ...
      const safeLimit = Math.min(
        Math.max(Number(reqCtx.query?.limit) || 50, 1),
        200
      );

      // 1. 验证会话归属权与有效性
      const convRes = await executeQuery<any>(
      ...
      const msgSql = `
        ...
        LIMIT ?;
      `;
      const msgRes = await executeQuery<any>(msgSql, [
        conversationId,
        currentUserId,
        safeLimit,
      ]);
      ...
      const messages = rowsInOrder.map((row: any) => {
