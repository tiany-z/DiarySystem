import {
  DEFAULT_TIANY_WALLPAPER_SETTINGS,
  getSystemSetting,
  returnError,
  returnSuccess,
  setSystemSetting,
  StandardResult,
  SystemWallpaperSettings,
...
export const api = {
  ...
  authRequired: false,
  run: null as any,
  handler: async (reqCtx: any, _ctx: any): Promise<StandardResult<any>> => {
    const method = (reqCtx.req.method || "GET").toUpperCase();

    // 1. GET 请求：全网公开读取管理员 tiany 的背景设置
    if (method === "GET") {
      const fetchRes = await getSystemSetting<SystemWallpaperSettings>(
        WALLPAPER_SETTING_KEY,
        DEFAULT_TIANY_WALLPAPER_SETTINGS
      );
    ...
    if (method === "POST" || method === "PUT") {
      ...
      if (!currentUser || currentUser.username !== "tiany") {
        return returnError(
          "权限不足：当前系统背景效果已由管理员 tiany 统一设定锁定，仅 tiany 账号具有调制保存权限"
        );
      }
      ...
      const payload: SystemWallpaperSettings = {
        enabled,
        blur,
        opacity,
        currentIndex,
