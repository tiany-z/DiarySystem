...
export function compileAstRunFunction(astConfig: any): (params: any, ctx?: any) => Promise<StandardResult<any>> {
  ...
  if (type === "INSERT") {
    ...
    const idColIndex = columns.findIndex((c: any) => c.column === "id");
    ...
    return async (params: any[], ctx?: any) => {
      try {
        ...
        const insertRes = await executeQuery<{ id: string | number; insertId?: number }>(sql, runParams);
  ...
  if (type === "UPDATE") {
    return async (params: any, ctx?: any) => {
      try {
        ...
        if (oldSnapshotRes.status === 0) return returnError(oldSnapshotRes.content);
