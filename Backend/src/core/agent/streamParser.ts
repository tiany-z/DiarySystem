...
export class ThinkingStreamParser {
  ...
  private hasProtocolReasoning: boolean = false;
   ...
   */
  public feedReasoningDelta(delta: string): void {
    if (!delta) return;
   ...
   */
  public feedContentDelta(delta: string): void {
    if (!delta) return;
    ...
    if (this.buffer) {
      if (this.mode === "THINKING") {
        ...
        if (closeIdx !== -1) {
          ...
          if (thoughtText) this.onThoughtChunk(thoughtText);
          if (remaining) this.onContentChunk(remaining);
        } else {
