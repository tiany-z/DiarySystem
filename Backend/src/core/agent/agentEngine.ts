...
export interface AgentExecutionOptions {
  ...
  callbacks: {
    ...
    onToolCallStart?: (toolCall: { id: string; name: string; args: any }) => void;
 ...
 */
export function normalizeCompletionsUrl(baseUrl: string): string {
...
export class AgentEngine {
  public static readonly MAX_ITERATIONS = 6;

  /**
   * 执行完整的 ReAct 决策循环
   */
  public async execute(options: AgentExecutionOptions): Promise<string> {
      ...
      for (let iteration = 1; iteration <= AgentEngine.MAX_ITERATIONS; iteration++) {
        ...
        if (isLastIteration) {
          workingMessages.push({
            role: "system",
            content:
              "【系统提示：当前已达到最大工具分析轮次，请根据此前获取到的所有检索数据与上下文，给出最终结论与完整答复。】",
          });
        }
        ...
        if (!response.ok) {
          ...
          throw new Error(
            `大模型接口响应异常 (HTTP ${response.status}): ${errBody.slice(0, 300) || response.statusText}`
          );
        }
