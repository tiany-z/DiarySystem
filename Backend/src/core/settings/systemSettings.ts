...
export const DEFAULT_TIANY_WALLPAPER_SETTINGS: SystemWallpaperSettings = {
  enabled: true,
  blur: 8,
  opacity: 0.35,
  currentIndex: 0,
 ...
 */
export async function ensureSystemSettingsTable(): Promise<StandardResult<boolean>> {
  try {
    ...
    const checkRes = await executeQuery<any>(checkSql, [WALLPAPER_SETTING_KEY]);
 ...
 * 获取系统键值设置
 */
export async function getSystemSetting<T>(key: string, defaultValue: T): Promise<StandardResult<T>> {
  try {
    const sql = "SELECT setting_value, updated_by, updated_at FROM system_settings WHERE setting_key = ? LIMIT 1;";
    const res = await executeQuery<any>(sql, [key]);
    if (res.status === 0) {
    ...
    try {
      ...
      return returnSuccess(res.data[0].setting_value as unknown as T);
    }
  } catch (error) {
 ...
 * 保存或更新系统键值设置
 */
export async function setSystemSetting<T>(key: string, value: T, updatedBy: string = "tiany"): Promise<StandardResult<T>> {
  try {
