...
describe("DiarySystem/Backend - Super Admin & User Management Lifecycle Tests", () => {
  ...
  it("公开注册通道 /api/auth/register 应已被拦截关闭", async () => {
    ...
    const res = await regRoute!.handler({ body: { username: "hack", password: "123" } }, {} as any);
  ...
  it("使用 tiany / Preservezty2004 登录应 100% 成功并签发 Token", async () => {
    ...
    const res = await loginRoute!.handler(
      { body: { username: SUPER_ADMIN_USERNAME, password: SUPER_ADMIN_DEFAULT_PASS } },
      {} as any
    );
    expect(res.status).toBe(1);
  ...
  it("非 tiany 用户访问 /api/admin/users/* 应被 403 权限拦截", async () => {
    ...
    const res = await listRoute!.handler({}, fakeCtx as any);
    expect(res.status).toBe(0);
  ...
  it("tiany 用户管理全流程：查看列表 -> 创建子账户 -> 修改密码 -> 验证登录 -> 级联删除连带笔记", async () => {
    ...
    // 1. 获取列表
    const listRes = await listRoute.handler({}, tianyCtx as any);
    ...
    const createRes = await createRoute.handler(
      { body: { username: subUsername, password: initialPass, nickname: "子账户测试" } },
      tianyCtx as any
    );
    ...
    // 3. 模拟为该子账户创建 2 篇笔记
    await executeQuery(
      "INSERT INTO diaries (id, user_id, title, content) VALUES (?, ?, ?, ?), (?, ?, ?, ?)",
      [
    ...
    const updateRes = await updatePassRoute.handler(
      { body: { userId: subUserId, newPassword: newPass } },
      tianyCtx as any
    );
    expect(updateRes.status).toBe(1);

    // 5. 验证新密码能登录
    const loginRes = await loginRoute.handler(
      { body: { username: subUsername, password: newPass } },
      {} as any
    );
    ...
    const checkDiaries = await executeQuery<{ count: number }>(
    ...
    // 验证用户无法再登录
    const loginAfterDel = await loginRoute.handler(
      { body: { username: subUsername, password: newPass } },
      {} as any
    );
